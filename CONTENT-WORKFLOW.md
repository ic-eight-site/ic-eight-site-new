...
4. For a cross-PVE Research Note, add `"contentType": "pve-research-note"` to the entry. Keep `tag` as the separate topic classification. Ordinary articles omit both `pve` and `contentType`.
5. Run:
...
If the registry contains a duplicate URL, duplicate title, duplicate PVE ID, unknown category or content type, an entry that combines `pve` with `contentType`, a missing article file, invalid publication timestamp, or a PVE card pointing to an unregistered article, generation stops without updating the derived files.
